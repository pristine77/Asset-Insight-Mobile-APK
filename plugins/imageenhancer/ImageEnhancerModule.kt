  ...
  fun autoEnhance(path: String, promise: Promise) {
    executor.execute {
      try {
        ...
        // Pass 1: contrast + saturation (fast, in-place row processing)
        ...
        // Pass 2: mild detail/sharpness (unsharp mask on luminance)
        val mp = (width.toLong() * height.toLong()).toDouble() / 1_000_000.0
        val doSharpen = width >= 3 && height >= 3 && mp <= 20.0
        ...
        if (doSharpen) {
          ...
          for (y in 1 until height - 1) {
            ...
            // slide window (keep ORIGINAL rows for luma blur computation)
            val tmpP = prev
            prev = curr
            curr = next
            next = tmpP
